...
- `RoomService` принудительно приводит статусы комнат и роли участников к Enum после каждого чтения из БД, чтобы исключить строки, возвращаемые драйвером PostgreSQL, и не допустить сбоев при переходе комнаты в `active`. 【F:apps/api/videochat_api/services/rooms.py†L52-L76】【F:apps/api/tests/test_rooms.py†L170-L214】
