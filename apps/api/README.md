...
3. **Настройте переменные окружения**:
   ...
   # добавьте ADMIN_SUPERUSERS (список email через запятую) для выдачи прав супер-админа
   ```
...
## Административный API и RBAC
- Ролевая модель (`user`, `moderator`, `admin`) хранится в таблице `user_roles`; при регистрации
  автоматически выдаётся роль `user`.
- Переменная окружения `ADMIN_SUPERUSERS` определяет список email/логинов с доступом к любым
  административным операциям независимо от записей в БД.
- Эндпоинты `/admin/users/*` позволяют листать пользователей, управлять блокировками и ролями.
  Для чтения достаточно роли `moderator`, для модификаций — `admin` или супер-пользователь.
- Действия записываются в `moderation_events`; при блокировке сессии пользователя отзываются
  автоматически. Заглушки `/admin/rooms/active` и `/admin/calls/active` зарезервированы под будущий мониторинг.
